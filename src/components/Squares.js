...
export default function Squares() {
  ...
  const getClassName = id => {
    ...
    return  (id = activeSquare) ? 'active' : nullState; 
  ...
  const markActive = id => {
    ...
    console.log ('active =', activeSquare);
    console.log ('clicked');
    console.log ('id =', id);
    (id != activeSquare) ? setActiveSquare(id) : setActiveSquare(nullState);
  };
      ...
      <div className='squares'>
        {
          ...
          squares.map(id =>
            <div
              id={id}
              key={id}
              className={`square ${(id === activeSquare) ? getClassName(id) : ""}`}
